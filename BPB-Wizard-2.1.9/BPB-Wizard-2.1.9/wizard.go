...
func downloadWorker() error {
	...
	for {
		...
		// Prefer local worker.js if available in the workspace (check exact BPB-Worker-Panel-4.0.1 (1) locations first)
		localPaths := []string{
			"./dist/worker.js",
			"../BPB-Worker-Panel-4.0.1 (1)/BPB-Worker-Panel-4.0.1/dist/worker.js",
			"../BPB-Worker-Panel-4.0.1 (1)/dist/worker.js",
			"../BPB-Worker-Panel-4.0.1/dist/worker.js",
			"./BPB-Worker-Panel/dist/worker.js",
			"../../BPB-Worker-Panel-4.0.1 (1)/BPB-Worker-Panel-4.0.1/dist/worker.js",
			"D:/create-vpn/BPB-Worker-Panel-4.0.1 (1)/BPB-Worker-Panel-4.0.1/dist/worker.js",
			"D:/create-vpn/BPB-Worker-Panel-4.0.1 (1)/dist/worker.js",
			"D:/create-vpn/BPB-Worker-Panel-4.0.1/dist/worker.js",
		}
		for _, p := range localPaths {
			if _, err := os.Stat(p); err == nil {
				// try to copy and append junk code as usual
				content, err := os.ReadFile(p)
				if err == nil {
					finalContent := append(content, []byte(generateJunkCode())...)
					if err := os.WriteFile(workerPath, finalContent, 0644); err == nil {
						successMessage(fmt.Sprintf("Using local worker.js from %s", p))
						return nil
					}
				}
			}
