...
async function buildConfig(
    ...
    const tag = isWarp ? `Nima Vpn | Warp - BestPing 🔁🔐` : "Nima Vpn | BestPing 🔁🔐";
    const mainUrlTest = buildUrlTest(tag, urlTestTags, isWarp);
    config.outbounds.push(mainUrlTest);
    if (isWarp) config.outbounds.push(buildUrlTest("Nima Vpn | WoW - BestPing 🔁🔐", secondUrlTestTags, isWarp));
    if (isChain) config.outbounds.push(buildUrlTest("Nima Vpn | BestPing 🔁🔐 🔗", secondUrlTestTags, isWarp));
