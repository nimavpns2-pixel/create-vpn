...
async function buildConfig(
    ...
    const name = isWarp ? `Nima Vpn | Warp ${isPro ? "Pro " : ""}- BestPing 🔁🔐` : "Nima Vpn | BestPing 🔁🔐";
    const mainUrlTest = buildUrlTest(name, proxyTags, isWarp);
    config["proxy-groups"].push(mainUrlTest);
    if (isWarp) config["proxy-groups"].push(buildUrlTest(`Nima Vpn | WoW ${isPro ? "Pro " : ""}- BestPing 🔁🔐`, chainTags, isWarp));
    if (isChain) config["proxy-groups"].push(buildUrlTest("Nima Vpn | BestPing 🔁🔐 🔗", chainTags, isWarp));
...
export async function getClNormalConfig(): Promise<Response> {
    ...
    const selectorTags = ["Nima Vpn | BestPing 🔁🔐"].concatIf(isChain, "Nima Vpn | BestPing 🔁🔐 🔗");
...
export async function getClWarpConfig(request: Request, env: Env, isPro: boolean): Promise<Response> {
    ...
    const selectorTags = [
        `Nima Vpn | Warp ${proSign}- BestPing 🔁🔐`,
        `Nima Vpn | WoW ${proSign}- BestPing 🔁🔐`
    ];

    warpEndpoints.forEach((endpoint, index) => {
        const warpTag = `Nima Vpn | VIP${index + 1}🔐 - Warp ${proSign}🇮🇷`;
        proxyTags.push(warpTag);

        const wowTag = `Nima Vpn | VIP${index + 1}🔐 - WoW ${proSign}🌍`;
